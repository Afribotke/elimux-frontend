    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-background rounded-xl border border-border p-5 shadow-sm">
          <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Total Ads</p>
          <p className="text-2xl font-bold text-foreground mt-1">{ads.length}</p>
        </div>
        <div className="bg-background rounded-xl border border-border p-5 shadow-sm">
        ...
        <div className="p-4">
          {filteredAds.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredAds.map((ad) => (
                <div key={ad.id} className="flex items-center gap-4 p-4 border border-border rounded-lg hover:bg-muted transition-colors">
                  {ad.image_url && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground truncate">{ad.placement} · {new Date(ad.start_date).toLocaleDateString()} - {new Date(ad.end_date).toLocaleDateString()}</p>
