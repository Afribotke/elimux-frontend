  ...
  return (
    <div className="mx-auto max-w-lg rounded-xl bg-white p-6 shadow-lg dark:bg-slate-900">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div><label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Amount</label><input type="number" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none dark:border-gray-600 dark:bg-slate-800 dark:text-white" min="0" step="0.01" required /></div>
      ...
      <PaymentModal isOpen={showModal} onClose={() => setShowModal(false)} amount={amount} currency={currency} description={description} metadata={{ email, phone }} />
    </div>
  )
}
