'use client'

import { useEffect, useState, useCallback } from 'react'
import Link from 'next/link'
import { supabase } from '@/lib/supabase'
import {
  listInstitutions,
  createInstitution,
  updateInstitution,
  deleteInstitution,
} from '@/lib/api'
import { useAdminKey } from '@/components/admin/AdminKeyContext'
import AddInstitutionForm, {
  type InstitutionFormData,
  type InstitutionFormInitialData,
} from '@/components/admin/AddInstitutionForm'
import { ArrowLeft, Building2, Pencil, Trash2, Plus, Search } from 'lucide-react'

interface InstitutionRow {
  id: string
  name: string
  type_id: string | null
  country_id: string | null
  city: string | null
  description: string | null
  website_url: string | null
  founded_year: number | null
  is_active: boolean
  type?: { name: string } | null
  country?: { name: string } | null
}

export default function AdminInstitutionsPage() {
  const { adminKey } = useAdminKey()
  const [institutions, setInstitutions] = useState<InstitutionRow[]>([])
  const [totalPages, setTotalPages] = useState(1)
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState('')
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [successMessage, setSuccessMessage] = useState<string | null>(null)

  const [types, setTypes] = useState<{ id: string; name: string }[]>([])
  const [countries, setCountries] = useState<{ id: string; name: string }[]>([])

  const [showForm, setShowForm] = useState(false)
  const [editing, setEditing] = useState<InstitutionFormInitialData | null>(null)

  useEffect(() => {
    async function loadReferenceData() {
      const [{ data: typeList }, { data: countryList }] = await Promise.all([
        supabase.from('institution_types').select('id, name').order('name'),
        supabase.from('countries').select('id, name').eq('is_active', true).order('name'),
      ])
      if (typeList) setTypes(typeList)
      if (countryList) setCountries(countryList)
    }
    loadReferenceData()
  }, [])

  const loadInstitutions = useCallback(async () => {
    setLoading(true)
    setError(null)
    try {
      const res = await listInstitutions({ page, limit: 20, search: search || undefined })
      setInstitutions(res.data)
      setTotalPages(res.meta.totalPages || 1)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load institutions')
    } finally {
      setLoading(false)
    }
  }, [page, search])

  useEffect(() => {
    loadInstitutions()
  }, [loadInstitutions])

  function flashSuccess(message: string) {
    setSuccessMessage(message)
    setTimeout(() => setSuccessMessage(null), 4000)
  }

  async function handleCreate(data: InstitutionFormData) {
    await createInstitution(data, adminKey)
    setShowForm(false)
    flashSuccess('Institution added successfully.')
    await loadInstitutions()
  }

  async function handleUpdate(data: InstitutionFormData) {
    if (!editing) return
    await updateInstitution(editing.id, data, adminKey)
    setEditing(null)
    flashSuccess('Institution updated successfully.')
    await loadInstitutions()
  }

  async function handleDelete(institution: InstitutionRow) {
    if (!window.confirm(`Delete "${institution.name}"? This cannot be undone.`)) return
    try {
      await deleteInstitution(institution.id, adminKey)
      flashSuccess('Institution deleted.')
      await loadInstitutions()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to delete institution')
    }
  }

  function openEdit(institution: InstitutionRow) {
    setEditing({
      id: institution.id,
      name: institution.name,
      type_id: institution.type_id || '',
      country_id: institution.country_id || '',
      city: institution.city || '',
      description: institution.description || '',
      website_url: institution.website_url || '',
      founded_year: institution.founded_year,
      is_active: institution.is_active,
    })
  }

  return (
    <main className="min-h-screen py-12 px-4 max-w-6xl mx-auto">
      <Link href="/admin" className="text-sm text-muted hover:text-foreground flex items-center gap-1 mb-4">
        <ArrowLeft className="w-4 h-4" /> Back to Dashboard
      </Link>

      <div className="flex items-center justify-between mb-2 flex-wrap gap-4">
        <h1 className="text-3xl font-bold text-foreground flex items-center gap-3">
          <Building2 className="w-8 h-8 text-primary-400" />
          Manage Institutions
        </h1>
        <button
          onClick={() => setShowForm(true)}
          className="px-4 py-2 rounded-lg bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium transition-colors flex items-center gap-2"
        >
          <Plus className="w-4 h-4" /> Add Institution
        </button>
      </div>

      <div className="relative mb-6 max-w-sm">
        <Search className="w-4 h-4 text-muted absolute left-3 top-1/2 -translate-y-1/2" />
        <input
          type="text"
          value={search}
          onChange={(e) => {
            setPage(1)
            setSearch(e.target.value)
          }}
          placeholder="Search institutions..."
          className="w-full pl-9 pr-3 py-2 text-sm rounded-lg bg-elimux-dark border border-border text-foreground focus:outline-none focus:border-primary-500"
        />
      </div>

      {successMessage && (
        <div className="mb-6 px-4 py-2 rounded-lg bg-elimux-success/10 border border-elimux-success/30 text-elimux-success text-sm">
          {successMessage}
        </div>
      )}

      {error && (
        <div className="mb-6 px-4 py-2 rounded-lg bg-elimux-danger/10 border border-elimux-danger/30 text-elimux-danger text-sm">
          {error}
        </div>
      )}

      {loading ? (
        <div className="text-center py-12">
          <div className="animate-spin w-8 h-8 border-2 border-primary-500 border-t-transparent rounded-full mx-auto mb-4" />
          <p className="text-muted">Loading institutions...</p>
        </div>
      ) : (
        <div className="bg-elimux-card border border-border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-elimux-dark text-muted text-left">
              <tr>
                <th className="px-4 py-3 font-medium">Name</th>
                <th className="px-4 py-3 font-medium">Type</th>
                <th className="px-4 py-3 font-medium">Country</th>
                <th className="px-4 py-3 font-medium">City</th>
                <th className="px-4 py-3 font-medium">Status</th>
                <th className="px-4 py-3 font-medium text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {institutions.map((inst) => (
                <tr key={inst.id} className="border-t border-border">
                  <td className="px-4 py-3 text-foreground font-medium">{inst.name}</td>
                  <td className="px-4 py-3 text-muted">{inst.type?.name || '—'}</td>
                  <td className="px-4 py-3 text-muted">{inst.country?.name || '—'}</td>
                  <td className="px-4 py-3 text-muted">{inst.city || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={inst.is_active ? 'text-elimux-success' : 'text-elimux-danger'}>
                      {inst.is_active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => openEdit(inst)}
                        className="p-1.5 rounded-lg hover:bg-muted/10 text-muted hover:text-foreground transition-colors"
                        aria-label="Edit"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDelete(inst)}
                        className="p-1.5 rounded-lg hover:bg-elimux-danger/10 text-muted hover:text-elimux-danger transition-colors"
                        aria-label="Delete"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                  </td>
                </tr>
              ))}
              {institutions.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-muted">
                    No institutions found.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      )}

      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 mt-6">
          <button
            onClick={() => setPage((p) => Math.max(1, p - 1))}
            disabled={page <= 1}
            className="px-3 py-1.5 rounded-lg border border-border text-sm text-muted disabled:opacity-40"
          >
            Previous
          </button>
          <span className="text-sm text-muted">Page {page} of {totalPages}</span>
          <button
            onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
            disabled={page >= totalPages}
            className="px-3 py-1.5 rounded-lg border border-border text-sm text-muted disabled:opacity-40"
          >
            Next
          </button>
        </div>
      )}

      {showForm && (
        <AddInstitutionForm
          types={types}
          countries={countries}
          onSubmit={handleCreate}
          onClose={() => setShowForm(false)}
        />
      )}

      {editing && (
        <AddInstitutionForm
          types={types}
          countries={countries}
          initialData={editing}
          onSubmit={handleUpdate}
          onClose={() => setEditing(null)}
        />
      )}
    </main>
  )
}
