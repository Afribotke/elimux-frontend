...
export default function AdminAdPricing() {
  ...
  if (!settings) return <div className="min-h-screen bg-muted flex items-center justify-center"><div className="text-red-600">Failed to load pricing settings. Check admin key.</div></div>;
  ...
  return (
    <div className="min-h-screen bg-muted">
      <div className="bg-background border-b border-border">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-foreground">Ad Pricing & Billing</h1>
              <p className="text-sm text-muted-foreground mt-1">Adjust CPC, CPM, CPA rates, fees, discounts. Changes apply instantly.</p>
            </div>
            <div className="flex items-center gap-3">
              {changed && <span className="px-3 py-1 bg-yellow-100 text-yellow-800 text-sm rounded-full">Unsaved Changes</span>}
              <button onClick={handleReset} disabled={!changed} className="px-4 py-2 border border-border rounded-lg text-sm disabled:opacity-50">Reset</button>
      ...
      <div className="max-w-5xl mx-auto px-6 mt-6">
        <div className="bg-background rounded-lg border border-border p-4 flex items-center justify-between">
          <div>
            <h3 className="font-medium text-foreground">Billing Mode</h3>
            <p className="text-sm text-muted-foreground">{settings.ad_billing_enabled ? "Per-click billing: advertisers pay only for actual clicks. No upfront charge." : "Flat budget mode: full budget deducted upfront at creation (legacy)."}</p>
          </div>
          <button onClick={() => handleChange("ad_billing_enabled", !settings.ad_billing_enabled)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.ad_billing_enabled ? "bg-blue-600" : "bg-secondary"}`}>
            <span className={`inline-block h-4 w-4 transform rounded-full bg-background transition-transform ${settings.ad_billing_enabled ? "translate-x-6" : "translate-x-1"}`} />
          </button>
        </div>
      </div>
        ...
        <div className="border-b border-border mb-6">
          <nav className="flex gap-6">
            {[{id:"rates",label:"Rates",icon:"$"},{id:"budgets",label:"Budgets",icon:"#"},{id:"fees",label:"Fees",icon:"%"},{id:"tiers",label:"Discounts",icon:"*"}].map((t) => (
              <button key={t.id} onClick={() => setTab(t.id)} className={`py-3 border-b-2 text-sm font-medium flex items-center gap-2 ${tab === t.id ? "border-blue-600 text-blue-600" : "border-transparent text-muted-foreground"}`}>{t.icon} {t.label}</button>
            ))}
          </nav>
        </div>
        ...
        {tab === "rates" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[{key:"ad_cpc_rate",label:"CPC Rate",desc:"Per click",color:"blue"},{key:"ad_cpm_rate",label:"CPM Rate",desc:"Per 1,000 impressions",color:"green"},{key:"ad_cpa_rate",label:"CPA Rate",desc:"Per conversion",color:"purple"}].map((f) => (
              <div key={f.key} className="bg-background rounded-lg border border-border p-5">
                ...
                <div className="relative">
                  ...
                  <input type="number" step="0.01" min="0.01" value={settings[f.key as keyof AdPricingSettings] as number} onChange={(e) => handleChange(f.key as keyof AdPricingSettings, e.target.value)} className="w-full pl-14 pr-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[{key:"ad_min_daily_budget",label:"Min Daily Budget"},{key:"ad_min_campaign_budget",label:"Min Campaign Budget"},{key:"ad_max_daily_budget",label:"Max Daily Budget"}].map((f) => (
              <div key={f.key} className="bg-background rounded-lg border border-border p-5">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">KES</span>
                  <input type="number" step="0.01" value={settings[f.key as keyof AdPricingSettings] as number} onChange={(e) => handleChange(f.key as keyof AdPricingSettings, e.target.value)} className="w-full pl-14 pr-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[{key:"ad_platform_fee_percent",label:"Platform Fee %",desc:"ElimuX keeps this %"},{key:"ad_partner_commission_percent",label:"Partner Commission %",desc:"Referral payout %"}].map((f) => (
              <div key={f.key} className="bg-background rounded-lg border border-border p-5">
                <label className="block text-sm font-medium text-foreground mb-2">{f.label}</label>
                <div className="relative">
                  <input type="number" step="0.01" min="0" max="100" value={settings[f.key as keyof AdPricingSettings] as number} onChange={(e) => handleChange(f.key as keyof AdPricingSettings, e.target.value)} className="w-full pr-10 pl-4 py-2 border border-border rounded-lg focus:ring-2 focus:ring-blue-500" />
        ...
        {tab === "tiers" && (
          <div className="space-y-4">
            {[{tier:1,threshKey:"ad_tier_1_threshold",discKey:"ad_tier_1_discount",label:"Bronze",color:"orange",emoji:"3"},{tier:2,threshKey:"ad_tier_2_threshold",discKey:"ad_tier_2_discount",label:"Silver",color:"gray",emoji:"2"},{tier:3,threshKey:"ad_tier_3_threshold",discKey:"ad_tier_3_discount",label:"Gold",color:"yellow",emoji:"1"}].map((t) => (
              <div key={t.tier} className="bg-background rounded-lg border border-border p-5 flex items-center gap-6">
                ...
                <div className="flex-1 grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">{t.label} Threshold</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">KES</span>
                      <input type="number" step="0.01" value={settings[t.threshKey as keyof AdPricingSettings] as number} onChange={(e) => handleChange(t.threshKey as keyof AdPricingSettings, e.target.value)} className="w-full pl-14 pr-4 py-2 border border-border rounded-lg" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">{t.label} Discount</label>
                    <div className="relative">
                      <input type="number" step="0.01" value={settings[t.discKey as keyof AdPricingSettings] as number} onChange={(e) => handleChange(t.discKey as keyof AdPricingSettings, e.target.value)} className="w-full pr-10 pl-4 py-2 border border-border rounded-lg" />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">%</span>
                    </div>
                  </div>
                </div>
