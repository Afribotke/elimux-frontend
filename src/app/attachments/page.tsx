...
import {
  Search, MapPin, Briefcase, Calendar, DollarSign, Users, Star, Filter, Zap, Clock, ChevronRight, Building2, GraduationCap
} from "lucide-react";
...
// Cycle 027 quick-fix: rebuilt to match /internships/page.tsx's design
// quality exactly (same hero/search/filter/card/grid structure), since
// this page had never received the same visual pass - it was still the
// original minimal dark-card list from before Cycle 025. Same query
// pattern as /internships (same `internships` table, `type` column just
// filters to 'attachment' instead), same singleton-supabase-client fix
// (see the comment on that state below), same AI match-score logic for
// structural parity, since real attachment listings will need the same
// filters/sort once employers start posting them via the type selector
// added in Cycle 026.
export default function AttachmentsPage() {
  ...
  const [activePlacements, setActivePlacements] = useState(0);
  ...
  const handleApply = (id: string) => router.push(`/attachments/${id}`);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-background/20 backdrop-blur-sm rounded-full px-4 py-2 mb-4">
              ...
              <span className="text-sm font-medium">{activePlacements} Active Placements Right Now</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Find Your Perfect Attachment</h1>
            <p className="text-xl text-emerald-100 max-w-2xl mx-auto">
              Industrial attachments arranged through your university. Get hands-on experience with
              verified employers in your field.
            </p>
          </div>
          <div className="max-w-3xl mx-auto">
            <div className="bg-background rounded-2xl shadow-xl p-2 flex items-center gap-2">
              ...
              <Input
                placeholder="Search by program, company or skill..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <span className="text-sm text-muted-foreground">Show paid attachments only</span>
                </div>
              </div>
              <div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-foreground">
            {loading ? "Loading..." : `${totalCount} Attachments Found`}
          </h2>
          {studentProfile && (
        ...
        {loading ? (
          ...
          <>
            <EmptyState
              ...
              title="No Attachments Available Yet"
              description="Industrial attachments are posted by employers and arranged through your university. Check back soon, or browse internships."
              action={
                <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={() => router.push("/internships")}>
                  Browse Internships<ChevronRight className="w-4 h-4 ml-1" />
                </Button>
              }
            />
            <Card className="mt-6">
              <CardContent className="p-8 text-center">
                <h3 className="text-xl font-semibold text-foreground mb-2">Are you an employer?</h3>
                <p className="text-muted-foreground mb-4">
                  Post attachment opportunities for verified university students.
                </p>
                <Button variant="outline" onClick={() => router.push("/employer/vacancies/new")}>
                  Post an Attachment
                </Button>
              </CardContent>
            </Card>
