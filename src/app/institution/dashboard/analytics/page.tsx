          ...
          <tbody>
            {links.map((link) => (
              <tr key={link.id} className="border-t border-gray-100">
                <td className="px-4 py-3 capitalize">{link.content_type}</td>
                <td className="px-4 py-3 text-gray-500">/s/{link.slug}</td>
                <td className="px-4 py-3 text-right font-medium">{link.total_clicks || 0}</td>
                <td className="px-4 py-3 text-right">{link.unique_clicks || 0}</td>
                <td className="px-4 py-3">{new Date(link.created_at).toLocaleDateString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
