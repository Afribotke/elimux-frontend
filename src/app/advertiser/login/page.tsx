...
// ============================================
// ELIMUX AD PORTAL - ADVERTISER LOGIN
// /advertiser/login
// ============================================
...
export default function AdvertiserLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If registration was submitted before an email confirmation completed,
    // finish creating the advertiser profile now that we have a session.
    const pending = takePendingAdvertiserRegistration()
    if (pending) {
      try {
        await advertiserFetch('/api/advertiser/register', {
          method: 'POST',
          body: JSON.stringify(pending),
        })
      } catch {
        // Non-fatal - dashboard will redirect to /advertiser/register if no profile exists.
      }
    }
      ...
      <div className="max-w-md w-full">
        <div className="bg-elimux-card rounded-xl border border-border overflow-hidden">
          <div className="px-8 py-6 border-b border-border">
            ...
            <p className="text-muted mt-1 text-sm">Sign in to manage your campaigns on ElimuX</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
            ...
            <p className="text-sm text-muted text-center">
              New advertiser?{' '}
              <Link href="/advertiser/register" className="text-primary-400 hover:text-primary-300">
