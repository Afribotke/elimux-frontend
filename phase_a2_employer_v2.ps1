...
Write-Host "`n=== PHASE A2 v2: EMPLOYER PORTAL + BULK UPLOAD + ACTIVATION ===" -ForegroundColor Cyan

# --- 1. Update employer registration API to set status = pending ---
Write-Host "[1/8] Updating employer registration API..." -ForegroundColor Yellow
$registerApi = Get-Content -LiteralPath "src/app/api/employers/register/route.ts" -Raw
if ($registerApi -notmatch "status.*pending") {
    $registerApi = $registerApi -replace "status: `"active`"", "status: `"pending`""
    Set-Content -LiteralPath "src/app/api/employers/register/route.ts" $registerApi -Force
    Write-Host "  Updated: self-registration now sets status = pending" -ForegroundColor Green
}

# --- 2. Create bulk upload API for admin ---
Write-Host "[2/8] Creating admin bulk upload API..." -ForegroundColor Yellow
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, created: results.length, failed: errors.length, results, errors })
  } catch (error: any) {
...
# --- 3. Create employer activation page ---
Write-Host "[3/8] Creating employer activation page..." -ForegroundColor Yellow
...
# --- 4. Create token verification API ---
Write-Host "[4/8] Creating token verification API..." -ForegroundColor Yellow
...
# --- 5. Create activation API ---
Write-Host "[5/8] Creating activation API..." -ForegroundColor Yellow
...
# --- 6. Create admin bulk upload page ---
Write-Host "[6/8] Creating admin bulk upload page..." -ForegroundColor Yellow
...
# --- 7. Update employer dashboard to show status warning ---
Write-Host "[7/8] Updating employer dashboard..." -ForegroundColor Yellow
$dashPath = "src/app/employer/dashboard/page.tsx"
$dashContent = Get-Content -LiteralPath $dashPath -Raw
if ($dashContent -notmatch "status") {
    $dashContent = $dashContent -replace "(<h1 className=`"text-3xl font-bold)", "<div className=`"mb-4 rounded-lg bg-yellow-50 p-4 text-sm text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400`">Your account status determines visibility. Only ACTIVE employers appear in public listings.</div>`n `$1"
    Set-Content -LiteralPath $dashPath $dashContent -Force
    Write-Host "  Added status notice to dashboard" -ForegroundColor Green
}

# --- 8. Build, commit, deploy ---
